...
private slots:
    ...
    void privateRuntimeDoesNotPersist();
...
void tst_persistenttabmodel::privateRuntimeDoesNotPersist()
{
    PrivateTabModel privateTabs(1001);
    ...
    QVERIFY(restoreSpy.first().first().value<PersistentTabRestoreBatch>().tabs().isEmpty());
    QVERIFY(DBManager::instance()->getSetting(QStringLiteral("desktopModeTabs")).isEmpty());
    QVERIFY(DBManager::instance()->getSetting(QStringLiteral("activeTabId")).isEmpty());
    QVERIFY(DBManager::instance()->getSetting(QStringLiteral("tabOrder")).isEmpty());
...
void tst_persistenttabmodel::runtimeCommandBoundaries()
{
    QSignalSpy activationSpy(tabModel, &DeclarativeTabModel::runtimeTabActivationRequested);
    QSignalSpy closeSpy(tabModel, &DeclarativeTabModel::runtimeTabCloseRequested);
    QSignalSpy clearSpy(tabModel, &DeclarativeTabModel::runtimeTabsClearRequested);
    tabModel->activateTab(0);
    ...
    addThreeTabs();
    QVERIFY(tabModel->activateTab(QStringLiteral("http://example.com")));
    QCOMPARE(activationSpy.last().at(0).toString(), QStringLiteral("1"));
    QVERIFY(!tabModel->activateTab(QStringLiteral("https://missing.example/")));
    QVERIFY(!tabModel->activateTabById(999));
    tabModel->activateTab(-1, true);
    QCOMPARE(activationSpy.last().at(0).toString(), QStringLiteral("1"));
    QCOMPARE(activationSpy.last().at(1).toBool(), true);
    tabModel->activateTab(999);
    QCOMPARE(activationSpy.last().at(0).toString(), QStringLiteral("3"));
    tabModel->remove(-1);
    tabModel->remove(999);
    QCOMPARE(closeSpy.count(), 0);
    tabModel->removeTabById(2, false);
    QCOMPARE(closeSpy.last().at(0).toString(), QStringLiteral("2"));
    tabModel->closeActiveTab();
    QCOMPARE(closeSpy.last().at(0).toString(), QStringLiteral("3"));
    QCOMPARE(tabModel->count(), 3);
    QCOMPARE(tabModel->activeTabId(), 3);
    QVERIFY(!tabModel->requestRuntimeTabNavigation(999, QStringLiteral("https://example.com")));
    QVERIFY(!tabModel->requestRuntimeTabNavigation(1, QString()));
}
...
void tst_persistenttabmodel::addThreeTabs()
{
    ...
    for (int i = 0; i < urls.count(); i++) {
        snapshot.append(QVariantMap {
            {QStringLiteral("tabId"), QString::number(100 + i)},
            {QStringLiteral("persistentId"), QString::number(i + 1)},
            {QStringLiteral("location"), urls.at(i)},
            {QStringLiteral("title"), titles.at(i)}
        });
    }
    tabModel->applyRuntimeSnapshot(snapshot, QStringLiteral("102"));
}
