...
(function() {
    ...
    var preserved = [
        ...
        request({ "data": { "promptId": "124" } }),
        request({ "message": "embed:auth" })
    ]
    ...
    for (var activeIndex = 0; activeIndex < activeRequests.length; ++activeIndex) {
        ...
        cancelHostedPrompt(host, "1", "10", { "promptId": "123" })
        ...
        if (calls.length) equal(calls[0][0], "embed:promptabort")
    }
    return true
})()
